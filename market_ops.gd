...
const OPERATIONS := {
	...
	"short": {"name": "SHORT & LEAK", "cost": 75, "heat": 12.0, "detail": "Selected venue -20%; collect 110 gold. Cash now, weaker Index."},
...
static func execute(operation: String, target: StringName) -> Dictionary:
	...
	match operation:
		...
		"short":
			asset.current_price = maxf(asset.current_price * 0.8, 0.01)
			RunEconomy.add_bonus(110)
		"hedge":
