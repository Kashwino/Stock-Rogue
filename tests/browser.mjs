...
try {
  ...
  assert.equal((await state()).profile, 'ghost', 'selected character enters heist');
  await tap('PAUSE');
  await tap('MENU - LAST CHECKPOINT');
  await wait(() => window.stockRogueQA?.scene.endsWith('home_screen.tscn'));
  await page.reload();
  await wait(() => window.stockRogueQA?.scene.endsWith('home_screen.tscn'));
  assert((await state()).unlocks.includes('room_training'), 'built room survives browser reload');
  await tap('PLAY');
  await wait(() => window.stockRogueQA?.scene.endsWith('character_select.tscn'));
  await tapCard();
  await wait(() => window.stockRogueQA?.scene.endsWith('prep_lobby.tscn'));
  assert.equal((await state()).profile, 'ghost', 'save file resumes through lobby with same character');
  await tap('CONTINUE RUN');
  await wait(() => window.stockRogueQA?.scene.endsWith('map_ui_screen.tscn'));
  assert.equal((await state()).route_step, 0, 'resume does not advance unfinished heist');
